## Welcome to DynIMC!
[Compiling DynIMC](#compile-dynimc) - for those that want the latest unreleased features
...
[Setup DynIMC](#setup-dynimc)

[Compile DynIMC](#compile-dynimc)

[Updating Your Repository](#updating-your-repository)

#### Setup Java
The Java JDK is used to compile DynIMC.
...
***
Gradle is used to execute the various build tasks when compiling DynIMC.
...
#### Setup Git
Git is used to clone DynIMC and update your local copy.
...
2. Navigate to a place where you want to download DynIMC's source (eg `C:\Development\Github\Minecraft\`) by executing `cd [folder location]`. This location is known as `mcdev` from now on.
3. Execute `git clone git@github.com:Lunatrius/DynIMC.git`. This will download DynIMC's source into `mcdev`.
4. Right now, you should have a directory that looks something like:

***
    mcdev
    \-DynIMC
        \-DynIMC's files (should have build.gradle)
***

#### Compile DynIMC
1. Execute `gradle setupDevWorkspace`. This sets up Forge and downloads the necessary libraries to build DynIMC. This might take some time, be patient.
    * You will generally only have to do this once until the Forge version in `build.properties` changes.
2. Execute `gradle build`. If you did everything right, `BUILD SUCCESSFUL` will be displayed after it finishes. This should be relatively quick.
    * If you see `BUILD FAILED`, check the error output (it should be right around `BUILD FAILED`), fix everything (if possible), and try again.
3. Go to `mcdev\DynIMC\build\libs`.
    * You should see a `.jar` file named `DynIMC-#.#.#-#.#.#.#-universal.jar`.
